...
def read_invoke_url(filename: str) -> str:
    """
    Reads the invoke url from a filename

    Args:
        filename (str): filename to read url from
    
    Returns:
        str: The string of the invoke url
    """
    try:
...
def run_infinite_post_data_loop():
    ...
    # base kafka api gateway URL
    script_path = Path(__file__).parent
    filepath = script_path / "../batch-ingestion-tf/kafka-invoke-url.txt"
    base_url = read_invoke_url(filename=filepath)
    
    # API gateway URLs to post to topics 
